...
class EvalDatasetLearnedReward(DatasetLearnedReward):
    ...
    def merge_dataset(self, other, traj_id_offset=None):
        ...
        if traj_id_offset is None:
            ...
            traj_id_offset = max(self.traj_lrs.keys()) + \
                1 if self.traj_lrs else 0
    ...
    def subgoal_reach_rates(self):
        ...
        gt_rates = {i: count / num_trajs for i,
                    count in enumerate(gt_reach_counts)}
        detected_rates = {i: count / num_trajs for i,
                          count in enumerate(detected_reach_counts)}
    ...
    def reward_metrics_to_file(self, out_dir):
        ...
        traj_rews = {traj_id: traj_lr.rewards for traj_id,
                     traj_lr in self.traj_lrs.items()}
        save_reward_metrics(traj_rews, os.path.join(
            out_dir, 'reward_metrics.json'), avg=True)
        save_reward_metrics(traj_rews, os.path.join(
            out_dir, 'reward_metrics_per_traj.json'), avg=False)

        if any(traj_lr.env_rewards is not None for traj_lr in self.traj_lrs.values()):
            traj_env_rews = {traj_id: traj_lr.env_rewards for traj_id,
                             traj_lr in self.traj_lrs.items() if traj_lr.env_rewards is not None}
            save_reward_metrics(traj_env_rews, os.path.join(
                out_dir, 'env_reward_metrics.json'), avg=True)
            save_reward_metrics(traj_env_rews, os.path.join(
                out_dir, 'env_reward_metrics_per_traj.json'), avg=False)
    ...
    if isinstance(sensor_data, dict):
        ...
        for cam_name, cam_data in sensor_data.items():
            if isinstance(cam_data, dict) and "rgb" in cam_data:
                ...
                frames_dict[cam_name] = cam_data["rgb"][0, ...]
...
def _save_reward_plot(plot_path, step_rewards, subgoal_idxs=None, env_rewards=None, detected_subgoal_idxs=None):
    ...
    try:
        ...
        generate_reward_plot(ax1, step_rewards, subgoal_idxs,
                             env_rewards, detected_subgoal_idxs, "Reward", fontsizes)
    ...
    try:
        ...
        with h5py.File(h5_path, 'w') as f:
            for episode_id, data in enumerate(episodes_data):
                ...
                f.create_dataset(
                    f'traj_{episode_id}/states', data=states_array, compression='gzip')

                actions_array = np.array(data['actions'], dtype=np.float32)
                f.create_dataset(
                    f'traj_{episode_id}/actions', data=actions_array, compression='gzip')

                if save_rgb:
                    for camera_name, frames in data['observations'].items():
                        obs_array = np.array(frames, dtype=np.uint8)
                        f.create_dataset(
                            f'traj_{episode_id}/obs/sensor_data/{camera_name}/rgb', data=obs_array, compression='gzip')

                rewards_array = np.array(data['rewards'], dtype=np.float32)
                f.create_dataset(
                    f'traj_{episode_id}/rewards', data=rewards_array, compression='gzip')
                ...
                for i in range(len(data['states'])):
                    subgoal_data.append(
                        [len([idx for idx in subgoal_idxs if idx <= i])])
                subgoal_array = np.array(subgoal_data, dtype=np.int32)
                f.create_dataset(
                    f'traj_{episode_id}/obs/extra/subgoal', data=subgoal_array, compression='gzip')
        ...
        logging.info(
            f"Saved trajectory H5 to {h5_path} and metadata to {json_path}")
    except Exception as e:
        env.close()
...
def _run_episode(model, env, episode_num, seed, video_path=None, save_video=True, save_rgb=False):
    ...
    actions = []
    rewards = []
    subgoal_reachs_gt = [np.nan for _ in range(
        MAX_SUBGOAL)]  # Initialize to np.nan
    env_rewards = None
    subgoal_reachs = [np.nan for _ in range(
        MAX_SUBGOAL)]  # Initialize to np.nan
    ...
    eval_traj_lr = EvalTrajectoryLearnedReward(
        ...
        env_rewards=np.array(
            env_rewards, dtype=np.float32) if env_rewards else None,
...
def _single_process_evaluation(pipe, checkpoint_dir, eval_env_config, args, ep_num_bounds, device, traj_dir=None):
    ...
    for episode_num in tqdm(range(start_ep_num, end_ep_num), desc="Evaluating", unit="episode", disable=args.no_progress_bar):
        ...
        # ! crucial to have (start, end) to have different seeds for different processes
        ...
        # Log episode info
        num_gt_subgoals = int(
            np.sum(~np.isnan(eval_traj_lr.subgoal_reachs_gt)))
        num_detected_subgoals = int(
            np.sum(~np.isnan(eval_traj_lr.subgoal_reachs)))
        ...
        logging.info(
            f"Episode {episode_num}: reward={np.sum(eval_traj_lr.rewards):.2f}, length={len(eval_traj_lr.rewards)}{add_info}")
...
def main():
    ...
    if config.reward_wrapper.type not in ["sparse", "env", "env_state-intrinsic"]:
        logging.info(
            f"Getting learned reward model for reward wrapper type {config.reward_wrapper.type}...")
        if args.learned_reward_model_path is not None:
            logging.info(
                f"Loading learned reward model from {args.learned_reward_model_path}...")
            learned_reward_data = utils.load_learned_reward_data(
                args.learned_reward_model_path, device=device, data_dir=args.learned_reward_data_dir)
        else:
            logging.info(
                f"Getting learned reward model path from config {config.reward_wrapper.pretrained_path}")
            learned_reward_data = utils.load_learned_reward_data(
                config.reward_wrapper.pretrained_path, device=device, data_dir=args.learned_reward_data_dir)
    else:
    ...
    results_dir = os.path.join(
        parent_dir, "out_eval-policy-py", model_basename)
    ...
    for proc_id in range(args.num_processes):
        logging.info(
            f"Starting evaluation on process {proc_id + 1}/{args.num_processes}...")
        ep_num_bounds = (proc_id * (args.num_episodes // args.num_processes),
                         (proc_id + 1) * (args.num_episodes // args.num_processes))
        parent_pipe, child_pipe = mp.Pipe()
        proc = mp.Process(target=_single_process_evaluation, args=(
            child_pipe, checkpoint_dir, eval_env_config, args, ep_num_bounds, device, traj_dir))
        proc.start()
        procs.append(proc)
    ...
    rewards_list = [
        traj_lr.rewards for traj_lr in eval_dataset.traj_lrs.values()]
    env_rewards_list = [traj_lr.env_rewards for traj_lr in eval_dataset.traj_lrs.values(
    ) if traj_lr.env_rewards is not None]
    ...
    mean_env_reward = float(
        np.mean([np.sum(r) for r in env_rewards_list])) if env_rewards_list else 0.0
    std_env_reward = float(
        np.std([np.sum(r) for r in env_rewards_list])) if env_rewards_list else 0.0
    ...
    logging.info(
        f"Min-Max Reward: {np.min(cumulative_rewards):.4f} - {np.max(cumulative_rewards):.4f}")
    logging.info(f"Success Rate: {success_rate:.4f}")

    logging.info(
        f"Average Subgoals Reached: {avg_subgoal_reached:.2f} / {MAX_SUBGOAL}")
    ...
    if env_rewards_list:
        logging.info(
            f"Mean Environment Reward: {mean_env_reward:.4f} ± {std_env_reward:.4f}")
        logging.info(
            f"Min-Max Environment Reward: {np.min([np.sum(r) for r in env_rewards_list]):.4f} - {np.max([np.sum(r) for r in env_rewards_list]):.4f}")
    ...
    _save_trajectory_h5(h5_path, json_traj_path, eval_env_config,
                        episodes_data, save_rgb=args.save_rgb)
