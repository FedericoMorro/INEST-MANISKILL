...
def compute_goal_embedding(model, downstream_loader, device):
...
def plot_results(rewards, output_dir, exp_name):
  ...
  plt.tight_layout()
  output_path = os.path.join(output_dir, f'{exp_name}_sample_trajs_reward.png')
  ...
  plt.tight_layout()
  output_path = os.path.join(output_dir, f'{exp_name}_mean_std_rewards.png')
  plt.savefig(output_path, dpi=300, bbox_inches='tight')
  plt.close()
...
def setup(experiment_path):
  """Load the latest embedder checkpoint and dataloaders"""

  config = utils.load_config_from_dir(experiment_path)
  ...
  return model, downstream_loaders, device
...
def main(args):
  ...
  model, downstream_loader, device = setup(args.experiment_path)
  
  # Compute goal embedding
  goal_emb, all_trajectories = compute_goal_embedding(model, downstream_loader, device)
  print(f"Goal embedding computed - shape: {goal_emb.shape}")
  
  # compute reward signals
  rewards = compute_reward_signals(all_trajectories, goal_emb)
...
if __name__ == "__main__":
  ...
  arg_parser.add_argument("--output_dir", type=str, default="out/reward_plots",
